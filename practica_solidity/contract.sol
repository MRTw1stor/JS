...
contract Seatransfer {
    ...
    // Функция добавления новых покупателей и администраторов в систему
    function NewUsers(address _user, uint _statusUser) public {
        ...
        for (uint i = 0; i < users.length; i++) {
            ...
            require(users[i].user != _user, "Such a buyer or administrator already exists");
        }
        ...
        if (_statusUser == 1) {
            users.push(Users( _user, false, false, true));
        }
    }
    ...
    // Функция отклонения или принятия заявок на понижение роли
    function AcceptOrIgnorRequest (uint id_request) public {
