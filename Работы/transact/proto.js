...
const contractAddress = "0x362Bd37ccCcF1443A980E8EfF61FF6A9A2f7DFAa";
...
let voteds = await myContract.methods.Check_Voted().call();
...
async function UpdateData() {
  ...
  voteds = await myContract.methods.Check_Voted().call();
}
...
document.getElementById("createuser").addEventListener("click", async () => {
  const usernameInput = document.getElementById("createnewusername");
  const useradressInput = document.getElementById("createnewuseradress");
...
async function Offers(offers, voteds) {
  RemoveItem();
  tableOffers.style.display = "block";
  const tableoffers = document.getElementById("tableOffers");

  for (let i = 0; i < offers.length; i++) {
    let foundVoted = voteds.find(voted => voted.voted === whoTransact.value && voted.id_up === offers[i].id_up);

    if (
      !offers[i].statusUp &&
      !offers[i].statusCansel &&
      !foundVoted
    ) {
      let body = document.createElement("tbody");
      ...
      let Button = document.createElement("button");
      Button.classList.add("buttonAccept");
      Button.id = `acept_${offers[i].id_up}`;
      Button.textContent = "Подтвердить";
      ...
      let button = document.createElement("button");
      button.classList.add("buttonCansel");
      button.id = `kansel_${offers[i].id_up}`;
...
document.getElementById("compUp").addEventListener("click", async () => { await Offers(offers, voteds) });
