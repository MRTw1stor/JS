...
// Адрес контракта
const contractAddress = "0xF8e730F43228dFf965Cb748507818f7eE3982d96";
...
async function compliteTransfer(array) {
  ...
  for (let i = 0; i < array[0].length; i++) {
    if (
      whoTransact.value == array[0][i].to &&
      array[0][i].status_execution == true &&
      array[0][i].status_reception == false
    ) {
      ...
      body.append(tdName, tdMoney, tdPasword, tdButton);
...
async function SentTransaction(array) {
  ...
  for (let i = 0; i < array[0].length; i++) {
    if (
      whoTransact.value == array[0][i].who &&
      array[0][i].status_execution == false &&
      array[0][i].status_reception == true
    ) {
      let body = document.createElement("tbody");
...
document
  .getElementById("comitTrans")
  .addEventListener("click", getInboxTransaction);
document
  .getElementById("sentTrans")
  .addEventListener("click", getSentTransaction);
document
  .getElementById("compliteTransfer")
  .addEventListener("click", compliteTransfer);
