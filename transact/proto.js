...
// Адрес контракта
const contractAddress = "0x4793Fedc8daA4E05Abfa8ABdfa1F03a734B9a370";
...
async function Transfer() {
  ...
  document.getElementById("gotransfer").addEventListener("click", async () => {
    ...
    if (code.value.trim() === "") {alert("Введите кодовое слово");} 
    if (accauntTo == accauntFrom) {alert("Вы не можете переводить деньги самому себе");} 
...
async function compliteTransfer(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    if (whoTransact.value == array[i].to) {
      ...
      document.getElementById("quicBtn").addEventListener("click", () => {body.remove();})
...
async function compliteMoneyTransfer(){
  const idTransfer = document.querySelectorAll(".input_list")
  for(const item of idTransfer) {
  ...
  await myContract.methods
    .CompliteTransfer(selectedAccount, convertMoney, web3.utils.fromAscii(code))
    .send({
      from: fromAddress,
      value: parseInt(convertMoney),
      gas: 1000000,
...
async function getAllTransfers() {
  const allTransfers = await myContract.methods
  .Check_Transfers()
  .call({
