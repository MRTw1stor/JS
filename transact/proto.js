...
const contractAddress = "0x58d4184DeAAa4D56b07E4cb1D3Bf2F3614Eb5432";
...
async function transferMoney() {
  ...
  const passwordHash = ethers.utils.formatBytes32String(codes);
...
async function compliteMoneyTransfer() {
  ...
  for (const btn of AcceptButtons) {
    ...
    for (const pass of password) {
      ...
      if (btn.id === pass.id) {
        console.log("56");
        const hashPass = ethers.utils.formatBytes32String(pass.value);
