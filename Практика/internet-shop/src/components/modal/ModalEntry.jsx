...
const ModalEntry = () => {
    ...
    const [disableModalEntry, setDisableModalEntry] = useState(false);
    ...
    const onLogin = () => {
        if(email === userItem.email && password === userItem.password) {
            localStorage.setItem("auth", JSON.stringify({ status: true }));
        } else return false;
    };

    return (
        <section className={disableModalEntry ? "modalentry_close" : null}>
            <div className={showModalRegistration ? "modalentry_close" : "modalentry"}>
                <div id="elipseentry"></div>
                <div id="otherentry">
                    <div id="formentry">
                        <div id="closeentry" onClick={() => setDisableModalEntry(!disableModalEntry)}>
                        ...
                        <div className="otherentry">
                            <div className="entrytext1">
                                <span id="entrytext1">Вход в личный кабинет</span>
                            </div>
                            <div>
                                <input id="entrymail" className="entryinput" type="text" placeholder="Электронная почта" onChange={(ev) => setEmail(ev.target.value)}/>
                            </div>
                            <div>
                                <input id="entrypassword" className="entryinput" type="text" placeholder="Пароль" onChange={(ev) => setPassword(ev.target.value)}/>
                            </div>
                            <div className="entrytext2">
                                <span id="entrytext2">Не помню пароль</span>
                            </div>
                            <div className="entrybutton">
                                <button id="entryenter" onClick={onLogin}>
                                    <span id="entryentertext">Войти в кабинет</span>
                                </button>
                                <button id="entryregistrate">
                                    <span id="entryregistratetext" onClick={() => setShowModalRegistration(!showModalRegistration)}>Регистрация</span>
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            {showModalRegistration ? <ModalRegistration /> : null};
        </section>
    )
}
