            ...
            if (!originalCellContent[rowId] || Object.keys(originalCellContent[rowId]).length === 0) {
                ...
                Array.from(row.children).slice(3).forEach((cell, index) => {
                    ...
                    if (index === 1 || index === 2) {}
                        ...
                        const changeInputStationContur = document.createElement("input");
                        changeInputStationContur.type = "text";
                        changeInputStationContur.id = "changeInputStationContur"
                        changeInputStationContur.value = cell.textContent.trim();

                        cell.textContent = "";
                        cell.appendChild(changeInputStationContur); 

                        changeInputStationContur.addEventListener('change', function(e) {
            ...
            document.getElementById("saveChangeButton").addEventListener('click', async () => {
                ...
                if (!statusReload){
                    if (inventoryNumberStatus){
                        ...
                        else if (newInventoryNumberValue.trim() !== "") {
                            setTimeout(() => {
                    ...
                    if (statusStatus && newStatusValue == oldStatusValue) {
                        alert("Новое значения поля Статус равно старому");
                        statusStatus = false
                        return statusStatus;
                    }
                    else if (statusStatus && newStatusValue != oldStatusValue) {
                        setTimeout(() => {
                    ...
                    if (ConturStatus) {
                        if (newConturValue.length > 26) {
                            alert('Слишком большое значение Контура станции');
                            return;
                        }
                        else {
                            setTimeout(() => {
                                updateWorkStationTechnik("workstation", parseInt(rowId), "station_contur", newConturValue)
                                ConturStatus = false
                                return ConturStatus;
                            }, 1000);
                        }
